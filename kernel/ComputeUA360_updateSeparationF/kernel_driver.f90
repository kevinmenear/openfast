    ...
    PROGRAM kernel_driver 
        ...
        IF (kgen_ierr_list .NE. 0) THEN 
            CALL SYSTEM("ls -1 ComputeUA360_updateSeparationF.*.*.* > kgen_statefile.lst") 
            CALL SLEEP(1) 
            kgen_unit_list = kgen_get_newunit() 
            OPEN (UNIT=kgen_unit_list, FILE="kgen_statefile.lst", STATUS="OLD", IOSTAT=kgen_ierr_list) 
        END IF   
        ...
        IF (myrank == 0) THEN 
            WRITE (*, *) "" 
            WRITE (*, "(A)") "****************************************************" 
            WRITE (*, "(4X,A)") "kernel execution summary: ComputeUA360_updateSeparationF" 
            WRITE (*, "(A)") "****************************************************" 
            ...
            ELSE 
                ...
                IF (kgen_case_count == kgen_count_verified) THEN 
                    WRITE (*, "(4X,A)") "kernel: ComputeUA360_updateSeparationF: PASSED verification" 
                ELSE 
                    WRITE (*, "(4X,A)") "kernel: ComputeUA360_updateSeparationF: FAILED verification" 
                END IF   
                WRITE (*, *) "" 
