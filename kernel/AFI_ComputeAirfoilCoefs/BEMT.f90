...
!----------------------------------------------------------------------------------------------------------------------------------
   SUBROUTINE bemt_calcoutput(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, u, p, afinfo, y) 
...
!..................................................................................................................................
       ...
       TYPE(bemt_parametertype), INTENT(INOUT) :: p 
      ...
      ! Local variables:
       ...
       TYPE(afi_outputtype) :: kgenref_afi_interp 
       ...
       CALL kr_airfoilinfo_types_afi_outputtype(afi_interp, kgen_unit, "afi_interp", .FALSE.) 
       ...
       CALL kr_airfoilinfo_types_afi_outputtype(kgenref_afi_interp, kgen_unit, "kgenref_afi_interp", .FALSE.) 
...
!!#endif
      ...
      ! Now depending on the option for UA get the airfoil coefs, Cl, Cd, Cm for unsteady or steady implementation
       ...
       !call to kgen kernel 
            ...
            IF (kgen_mainstage) THEN 
                ...
                CALL kv_bemt_calcoutput_character_errmsglen_("errmsg2", check_status, errmsg2, kgenref_errmsg2) 
                CALL kv_bemt_calcoutput_integer__intki("errstat2", check_status, errstat2, kgenref_errstat2) 
                CALL kv_airfoilinfo_types_afi_outputtype("afi_interp", check_status, afi_interp, kgenref_afi_interp) 
            ...
            call AFI_ComputeAirfoilCoefs( y%AOA(i,j), y%Re(i,j), u%UserProp(i,j),  AFInfo(p%AFindx(i,j)), AFI_interp, errStat2, errMsg2 )
                ...
                IF (check_status%rank==0) THEN 
                    WRITE (*, *) "AFI_ComputeAirfoilCoefs : Time per call (usec): ", kgen_measure 
                END IF   
            END IF   
            IF (kgen_warmupstage) THEN 
            END IF   
   ...
   !............................................
            ...
            !verify state subroutine for kv_bemt_calcoutput_character_errmsglen_ 
            RECURSIVE SUBROUTINE kv_bemt_calcoutput_character_errmsglen_(varname, check_status, var, kgenref_var) 
        ...
        IF (var /= var) WRITE(*, *) trim(adjustl(varname))," is IDENTICAL (both NaN, uninitialized)." 
            ...
            END SUBROUTINE kv_bemt_calcoutput_character_errmsglen_ 
              
            !verify state subroutine for kv_bemt_calcoutput_integer__intki 
            RECURSIVE SUBROUTINE kv_bemt_calcoutput_integer__intki(varname, check_status, var, kgenref_var) 
                CHARACTER(LEN=*), INTENT(IN) :: varname 
                TYPE(check_t), INTENT(INOUT) :: check_status 
        ...
        IF (var /= var) WRITE(*, *) trim(adjustl(varname))," is IDENTICAL (both NaN, uninitialized)." 
            ...
            END SUBROUTINE kv_bemt_calcoutput_integer__intki 
