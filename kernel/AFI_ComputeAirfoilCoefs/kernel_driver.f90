    ...
    PROGRAM kernel_driver 
        ...
        USE bemt, ONLY: bemt_calcoutput 
          
        USE bemt_types, ONLY: bemt_inputtype 
        USE bemt_types, ONLY: kr_bemt_types_bemt_inputtype 
        USE bemt_types, ONLY: bemt_parametertype 
        USE bemt_types, ONLY: kr_bemt_types_bemt_parametertype 
        USE bemt_types, ONLY: afi_parametertype 
        USE bemt_types, ONLY: kr_airfoilinfo_types_afi_parametertype 
        USE bemt_types, ONLY: bemt_outputtype 
        USE bemt_types, ONLY: kr_bemt_types_bemt_outputtype 
        USE nwtc_base, ONLY: kr_externs_in_nwtc_base 
        ...
        IF (kgen_ierr_list .NE. 0) THEN 
            CALL SYSTEM("ls -1 AFI_ComputeAirfoilCoefs.*.*.* > kgen_statefile.lst") 
            CALL SLEEP(1) 
            kgen_unit_list = kgen_get_newunit() 
            OPEN (UNIT=kgen_unit_list, FILE="kgen_statefile.lst", STATUS="OLD", IOSTAT=kgen_ierr_list) 
        END IF   
        ...
        DO WHILE ( kgen_ierr_list .EQ. 0 ) 
            ...
            IF (kgen_ierr_list .EQ. 0) THEN 
                ...
                IF (kgen_ierr == 0) THEN 
                    ...
                    CALL kr_bemt_types_bemt_inputtype(u, kgen_unit, "u", .FALSE.) 
                    CALL kr_bemt_types_bemt_parametertype(p, kgen_unit, "p", .FALSE.) 
                    CALL kr_kgen_bemt_calcoutput_subp2(afinfo, kgen_unit, "afinfo", .FALSE.) 
                    CALL kr_bemt_types_bemt_outputtype(y, kgen_unit, "y", .FALSE.) 
                      
                    !extern input variables 
                    CALL kr_externs_in_nwtc_base(kgen_unit) 
                    CALL kr_externs_in_nwtc_num(kgen_unit) 
                      
                    !callsite part 
                    CALL bemt_calcoutput(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, u, p, afinfo, y) 
        ...
        IF (myrank == 0) THEN 
            WRITE (*, *) "" 
            WRITE (*, "(A)") "****************************************************" 
            WRITE (*, "(4X,A)") "kernel execution summary: AFI_ComputeAirfoilCoefs" 
            WRITE (*, "(A)") "****************************************************" 
            ...
            ELSE 
                ...
                IF (kgen_case_count == kgen_count_verified) THEN 
                    WRITE (*, "(4X,A)") "kernel: AFI_ComputeAirfoilCoefs: PASSED verification" 
                ELSE 
                    WRITE (*, "(4X,A)") "kernel: AFI_ComputeAirfoilCoefs: FAILED verification" 
                END IF   
                WRITE (*, *) "" 
        ...
        !read state subroutine for kr_kgen_bemt_calcoutput_subp2 
        SUBROUTINE kr_kgen_bemt_calcoutput_subp2(var, kgen_unit, printname, printvar) 
            TYPE(afi_parametertype), INTENT(INOUT), ALLOCATABLE, DIMENSION(:) :: var 
            ...
            IF (kgen_istrue) THEN 
                ...
                DO idx1=kgen_bound(1,1), kgen_bound(2,1) 
                    IF (PRESENT( printvar ) .AND. printvar) THEN 
                        CALL kr_airfoilinfo_types_afi_parametertype(var(idx1), kgen_unit, printname // "(idx1)", .TRUE.) 
                    ELSE 
                        CALL kr_airfoilinfo_types_afi_parametertype(var(idx1), kgen_unit, printname // "(idx1)", .FALSE.) 
                    END IF   
                END DO   
            END IF   
        END SUBROUTINE kr_kgen_bemt_calcoutput_subp2 
