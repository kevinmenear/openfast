    ...
    PROGRAM kernel_driver 
        ...
        USE airfoilinfo, ONLY: afi_computeairfoilcoefs2d 
        ...
        REAL(KIND=reki) :: secondarydepval 
        ...
        IF (kgen_ierr_list .NE. 0) THEN 
            CALL SYSTEM("ls -1 FindBoundingTables.*.*.* > kgen_statefile.lst") 
            CALL SLEEP(1) 
            kgen_unit_list = kgen_get_newunit() 
            OPEN (UNIT=kgen_unit_list, FILE="kgen_statefile.lst", STATUS="OLD", IOSTAT=kgen_ierr_list) 
        END IF   
        ...
        DO WHILE ( kgen_ierr_list .EQ. 0 ) 
            ...
            IF (kgen_ierr_list .EQ. 0) THEN 
                ...
                IF (kgen_ierr == 0) THEN 
                    ...
                    READ (UNIT = kgen_unit) secondarydepval 
                    CALL kr_airfoilinfo_types_afi_parametertype(p, kgen_unit, "p", .FALSE.) 
                      
                    !extern input variables 
                      
                    !callsite part 
                    CALL afi_computeairfoilcoefs2d(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, secondarydepval, p) 
        ...
        IF (myrank == 0) THEN 
            WRITE (*, *) "" 
            WRITE (*, "(A)") "****************************************************" 
            WRITE (*, "(4X,A)") "kernel execution summary: FindBoundingTables" 
            WRITE (*, "(A)") "****************************************************" 
            ...
            ELSE 
                ...
                IF (kgen_case_count == kgen_count_verified) THEN 
                    WRITE (*, "(4X,A)") "kernel: FindBoundingTables: PASSED verification" 
                ELSE 
                    WRITE (*, "(4X,A)") "kernel: FindBoundingTables: FAILED verification" 
                END IF   
                WRITE (*, *) "" 
