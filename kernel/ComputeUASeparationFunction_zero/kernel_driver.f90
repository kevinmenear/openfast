    ...
    !Generated at : 2026-04-25 13:59:13 
    !KGEN version : 0.8.1 
    ...
    PROGRAM kernel_driver 
        ...
        INTEGER(KIND=intki) :: coluaf 
        INTEGER(KIND=intki) :: colcl 
        myrank = 0 
        mpisize = 1 
        ...
        IF (kgen_ierr_list .NE. 0) THEN 
            CALL SYSTEM("ls -1 ComputeUASeparationFunction_zero.*.*.* > kgen_statefile.lst") 
            CALL SLEEP(1) 
            kgen_unit_list = kgen_get_newunit() 
            OPEN (UNIT=kgen_unit_list, FILE="kgen_statefile.lst", STATUS="OLD", IOSTAT=kgen_ierr_list) 
        END IF   
        ...
        DO WHILE ( kgen_ierr_list .EQ. 0 ) 
            ...
            IF (kgen_ierr_list .EQ. 0) THEN 
                ...
                IF (kgen_ierr == 0) THEN 
                    ...
                    READ (UNIT = kgen_unit) coluaf 
                    READ (UNIT = kgen_unit) colcl 
                      
                    !extern input variables 
                    CALL kr_externs_in_nwtc_num(kgen_unit) 
                      
                    !callsite part 
                    CALL computeuaseparationfunction_oncl(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, p, coluaf, &
                    &colcl) 
        ...
        IF (myrank == 0) THEN 
            WRITE (*, *) "" 
            WRITE (*, "(A)") "****************************************************" 
            WRITE (*, "(4X,A)") "kernel execution summary: ComputeUASeparationFunction_zero" 
            WRITE (*, "(A)") "****************************************************" 
            ...
            ELSE 
                ...
                IF (kgen_case_count == kgen_count_verified) THEN 
                    WRITE (*, "(4X,A)") "kernel: ComputeUASeparationFunction_zero: PASSED verification" 
                ELSE 
                    WRITE (*, "(4X,A)") "kernel: ComputeUASeparationFunction_zero: FAILED verification" 
                END IF   
                WRITE (*, *) "" 
