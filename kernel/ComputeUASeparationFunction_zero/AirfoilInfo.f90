...
!----------------------------------------------------------------------------------------------------------------------------------
SUBROUTINE computeuaseparationfunction_oncl(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, p, coluaf, colcl) 
    USE kgen_utils_mod
    USE kgen_utils_mod
    ...
    !call to kgen kernel 
               ...
               call ComputeUASeparationFunction_zero(p, ColUAf, p%Coefs(:,ColCl)) ! this was comparing with alpha0, but now we compare with alphaUpper and alphaLower
                   ...
                   IF (check_status%rank==0) THEN 
                       WRITE (*, *) "ComputeUASeparationFunction_zero : Time per call (usec): ", kgen_measure 
                   END IF   
               END IF   
               IF (kgen_warmupstage) THEN 
               END IF   
