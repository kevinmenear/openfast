    ...
    !Generated at : 2026-04-24 12:51:07 
    !KGEN version : 0.8.1 
    ...
    PROGRAM kernel_driver 
        ...
        IF (kgen_ierr_list .NE. 0) THEN 
            CALL SYSTEM("ls -1 Calculate_Cn.*.*.* > kgen_statefile.lst") 
            CALL SLEEP(1) 
            kgen_unit_list = kgen_get_newunit() 
            OPEN (UNIT=kgen_unit_list, FILE="kgen_statefile.lst", STATUS="OLD", IOSTAT=kgen_ierr_list) 
        END IF   
        ...
        DO WHILE ( kgen_ierr_list .EQ. 0 ) 
            ...
            IF (kgen_ierr_list .EQ. 0) THEN 
                ...
                IF (kgen_ierr == 0) THEN 
                    ...
                    READ (UNIT = kgen_unit) colcd 
                      
                    !extern input variables 
                      
                    !callsite part 
                    CALL calculateuacoeffs(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, p, colcl, colcd) 
        ...
        IF (myrank == 0) THEN 
            WRITE (*, *) "" 
            WRITE (*, "(A)") "****************************************************" 
            WRITE (*, "(4X,A)") "kernel execution summary: Calculate_Cn" 
            WRITE (*, "(A)") "****************************************************" 
            ...
            ELSE 
                ...
                IF (kgen_case_count == kgen_count_verified) THEN 
                    WRITE (*, "(4X,A)") "kernel: Calculate_Cn: PASSED verification" 
                ELSE 
                    WRITE (*, "(4X,A)") "kernel: Calculate_Cn: FAILED verification" 
                END IF   
                WRITE (*, *) "" 
