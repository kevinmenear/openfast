    ...
    !Generated at : 2026-04-26 02:58:07 
    !KGEN version : 0.8.1 
      
    PROGRAM kernel_driver 
        USE kgen_utils_mod
        USE tprof_mod, ONLY: tstart, tstop, tnull, tprnt 
        USE airfoilinfo, ONLY: readaffile 
          
        USE airfoilinfo_types, ONLY: afi_initinputtype 
        USE airfoilinfo_types, ONLY: kr_airfoilinfo_types_afi_initinputtype 
        USE airfoilinfo_types, ONLY: afi_parametertype 
        USE airfoilinfo_types, ONLY: kr_airfoilinfo_types_afi_parametertype 
        ...
        TYPE(afi_initinputtype) :: initinp 
        ...
        IF (kgen_ierr_list .NE. 0) THEN 
            CALL SYSTEM("ls -1 CalculateUACoeffs.*.*.* > kgen_statefile.lst") 
            CALL SLEEP(1) 
            kgen_unit_list = kgen_get_newunit() 
            OPEN (UNIT=kgen_unit_list, FILE="kgen_statefile.lst", STATUS="OLD", IOSTAT=kgen_ierr_list) 
        END IF   
        ...
        DO WHILE ( kgen_ierr_list .EQ. 0 ) 
            ...
            IF (kgen_ierr_list .EQ. 0) THEN 
                ...
                IF (kgen_ierr == 0) THEN 
                    ...
                    CALL kr_airfoilinfo_types_afi_initinputtype(initinp, kgen_unit, "initinp", .FALSE.) 
                    CALL kr_airfoilinfo_types_afi_parametertype(p, kgen_unit, "p", .FALSE.) 
                    ...
                    !callsite part 
                    CALL readaffile(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, initinp, p) 
        ...
        IF (myrank == 0) THEN 
            WRITE (*, *) "" 
            WRITE (*, "(A)") "****************************************************" 
            WRITE (*, "(4X,A)") "kernel execution summary: CalculateUACoeffs" 
            WRITE (*, "(A)") "****************************************************" 
            ...
            ELSE 
                ...
                IF (kgen_case_count == kgen_count_verified) THEN 
                    WRITE (*, "(4X,A)") "kernel: CalculateUACoeffs: PASSED verification" 
                ELSE 
                    WRITE (*, "(4X,A)") "kernel: CalculateUACoeffs: FAILED verification" 
                END IF   
                WRITE (*, *) "" 
