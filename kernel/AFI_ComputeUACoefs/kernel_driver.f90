    ...
    PROGRAM kernel_driver 
        ...
        USE unsteadyaero, ONLY: ua_calccontstatederiv 
          
        USE unsteadyaero_types, ONLY: afi_parametertype 
        USE unsteadyaero_types, ONLY: kr_airfoilinfo_types_afi_parametertype 
        ...
        IF (kgen_ierr_list .NE. 0) THEN 
            CALL SYSTEM("ls -1 AFI_ComputeUACoefs.*.*.* > kgen_statefile.lst") 
            CALL SLEEP(1) 
            kgen_unit_list = kgen_get_newunit() 
            OPEN (UNIT=kgen_unit_list, FILE="kgen_statefile.lst", STATUS="OLD", IOSTAT=kgen_ierr_list) 
        END IF   
        ...
        DO WHILE ( kgen_ierr_list .EQ. 0 ) 
            ...
            IF (kgen_ierr_list .EQ. 0) THEN 
                ...
                IF (kgen_ierr == 0) THEN 
                    ...
                    CALL kr_airfoilinfo_types_afi_parametertype(afinfo, kgen_unit, "afinfo", .FALSE.) 
                      
                    !extern input variables 
                    CALL kr_externs_in_nwtc_num(kgen_unit) 
                      
                    !callsite part 
                    CALL ua_calccontstatederiv(kgen_unit, kgen_measure, kgen_isverified, kgen_filepath, afinfo) 
        ...
        IF (myrank == 0) THEN 
            WRITE (*, *) "" 
            WRITE (*, "(A)") "****************************************************" 
            WRITE (*, "(4X,A)") "kernel execution summary: AFI_ComputeUACoefs" 
            WRITE (*, "(A)") "****************************************************" 
            ...
            ELSE 
                ...
                IF (kgen_case_count == kgen_count_verified) THEN 
                    WRITE (*, "(4X,A)") "kernel: AFI_ComputeUACoefs: PASSED verification" 
                ELSE 
                    WRITE (*, "(4X,A)") "kernel: AFI_ComputeUACoefs: FAILED verification" 
                END IF   
                WRITE (*, *) "" 
